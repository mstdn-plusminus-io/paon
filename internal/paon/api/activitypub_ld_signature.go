...
func (s *Server) activityPubLinkedDataSignatureActor(body []byte, payload activityPayload) (*models.Account, error) {
	...
	if activityPubHasUnsupportedSignedJSONLDFeature(document) {
		return nil, fmt.Errorf("unsupported graph-restructuring linked-data signature document")
	}
	...
	if err := activityPubResolvedKeypairValidityError(payload.Signature.Creator, resolved, time.Now().UTC()); err != nil {
		return nil, fmt.Errorf("validate linked-data signature creator keypair: %w", err)
	}
