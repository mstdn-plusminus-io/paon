...
func TestUnverifiedActivityRefetchWorkerIntegration(t *testing.T) {
	...
	t.Run("Update actor fragment fetches the actor document", func(t *testing.T) {
		...
		if saved.DisplayName != "current origin profile" || saved.PublicKey != publicKey || *requests != 1 {
			t.Fatalf("updated actor name=%q key preserved=%v requests=%d", saved.DisplayName, saved.PublicKey == publicKey, *requests)
		}
	})
