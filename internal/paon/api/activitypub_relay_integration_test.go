...
func TestRelayedMisskeyActivityAuthenticatesBeforeForwardingFinalization(t *testing.T) {
	...
	var fetchedKeypair models.Keypair
	if err := database.Where("uri = ? AND account_id = ?", misskeyDeleteKeyID, fetchedCreator.ID).First(&fetchedKeypair).Error; err != nil || fetchedKeypair.PublicKey != misskeyDeletePublicKeyPEM {
		t.Fatalf("fetched signature creator did not retain the verified keypair: %v", err)
	}
