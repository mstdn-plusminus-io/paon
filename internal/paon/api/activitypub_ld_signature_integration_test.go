...
func TestRelayedMisskeySquareDeleteResolvesUnknownSignatureCreator(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(tt.wantError) == 0 {
				if len(actors) != 1 || actors[0].PublicKey != misskeySquareDeletePublicKeyPEM {
					t.Fatalf("resolved creator did not retain the expected public key: %d actors", len(actors))
				}
