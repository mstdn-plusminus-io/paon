...
func TestAdminSettingsDiscoveryHTMLIncludesRailsFields(t *testing.T) {
	html := adminSettingsDiscoveryHTML(adminDiscoverySettings{
		...
		LocalLiveFeedAccess:       timelineAccessPublic,
		RemoteLiveFeedAccess:      timelineAccessAuthenticated,
		LocalTopicFeedAccess:      timelineAccessPublic,
		RemoteTopicFeedAccess:     timelineAccessDisabled,
		NoIndex:                   true,
	...
	for _, want := range []string{
		...
		`name="form_admin_settings[local_live_feed_access]"`,
		`name="form_admin_settings[remote_live_feed_access]"`,
		`<option value="authenticated" selected>`,
		`name="form_admin_settings[local_topic_feed_access]"`,
		`name="form_admin_settings[remote_topic_feed_access]"`,
		`<option value="disabled" selected>`,
		`name="form_admin_settings[noindex]" value="1" checked`,
