...
func ensureMastodon44FinalAdditiveCatalog(tx *gorm.DB) error {
	statements := []string{
		`CREATE TABLE IF NOT EXISTS fasp_follow_recommendations (id bigserial PRIMARY KEY, requesting_account_id bigint NOT NULL, recommended_account_id bigint NOT NULL, created_at timestamp without time zone NOT NULL, updated_at timestamp without time zone NOT NULL, FOREIGN KEY (requesting_account_id) REFERENCES accounts(id), FOREIGN KEY (recommended_account_id) REFERENCES accounts(id))`,
...
func mastodon44ExpandSteps() []upgradeStep {
	return []upgradeStep{
		{version: "20240918233930", phase: "expand", statements: []string{`ALTER TABLE statuses ADD COLUMN fetched_replies_at timestamp without time zone`}},
		...
		{version: "20241111141355", phase: "expand", statements: []string{`CREATE TABLE tag_trends (id bigserial PRIMARY KEY, tag_id bigint NOT NULL, score double precision DEFAULT 0 NOT NULL, rank integer DEFAULT 0 NOT NULL, allowed boolean DEFAULT false NOT NULL, language character varying DEFAULT '' NOT NULL, FOREIGN KEY (tag_id) REFERENCES tags(id) ON DELETE CASCADE)`, `CREATE UNIQUE INDEX index_tag_trends_on_tag_id_and_language ON tag_trends (tag_id, language)`}},
		{version: "20241123224956", phase: "expand", statements: []string{`CREATE TABLE terms_of_services (id bigserial PRIMARY KEY, text text DEFAULT '' NOT NULL, changelog text DEFAULT '' NOT NULL, published_at timestamp without time zone, notification_sent_at timestamp without time zone, created_at timestamp without time zone NOT NULL, updated_at timestamp without time zone NOT NULL)`}},
		...
		{version: "20250103131909", phase: "expand", statements: []string{`CREATE TABLE fasp_backfill_requests (id bigserial PRIMARY KEY, category character varying NOT NULL, max_count integer DEFAULT 100 NOT NULL, cursor character varying, fulfilled boolean DEFAULT false NOT NULL, fasp_provider_id bigint NOT NULL, created_at timestamp without time zone NOT NULL, updated_at timestamp without time zone NOT NULL, FOREIGN KEY (fasp_provider_id) REFERENCES fasp_providers(id))`, `CREATE INDEX index_fasp_backfill_requests_on_fasp_provider_id ON fasp_backfill_requests (fasp_provider_id)`}},
		...
		{version: "20250221143646", phase: "expand", statements: []string{`ALTER TABLE announcements ADD COLUMN notification_sent_at timestamp without time zone`}},
		...
		{version: "20250313123400", phase: "expand", statements: []string{`ALTER TABLE users ADD COLUMN age_verified_at timestamp without time zone`}},
		...
		{version: "20250520204643", phase: "expand", statements: []string{`CREATE TABLE rule_translations (id bigserial PRIMARY KEY, text text DEFAULT '' NOT NULL, hint text DEFAULT '' NOT NULL, language character varying NOT NULL, rule_id bigint NOT NULL, created_at timestamp without time zone NOT NULL, updated_at timestamp without time zone NOT NULL, FOREIGN KEY (rule_id) REFERENCES rules(id) ON DELETE CASCADE)`, `CREATE UNIQUE INDEX index_rule_translations_on_rule_id_and_language ON rule_translations (rule_id, language)`}},
...
func mastodon44ContractSteps() []upgradeStep {
	return []upgradeStep{
		...
		{version: "20250627132728", phase: "contract", statements: []string{`INSERT INTO ar_internal_metadata (key, value, created_at, updated_at) VALUES ('schema_sha1', 'b53e3b8de778cd1b53158326b97afa9368f3237e', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = EXCLUDED.updated_at`}},
	}
}
