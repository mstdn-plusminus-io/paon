...
// reconcileCurrentMastodonCatalog only repairs details whose canonical value
// is independent of how a database reached the supported Mastodon release.
// Physical layout and route-specific sequence state are intentionally not
// inferred: long-lived and dump/restored databases can legitimately differ.
func reconcileCurrentMastodonCatalog(tx *gorm.DB) (bool, error) {
	functionChanged, err := reconcileTimestampIDFunction(tx)
	if err != nil {
		return false, err
	}
	foreignKeysChanged, err := reconcileMastodonForeignKeyNames(tx)
	if err != nil {
		return false, err
	}
...
func reconcileMastodonForeignKeyNames(tx *gorm.DB) (bool, error) {
	applied := false
