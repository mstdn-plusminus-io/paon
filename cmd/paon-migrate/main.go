...
func main() {
	...
	phase := flag.String("phase", "", "upgrade a supported Mastodon schema through expand, backfill, validate, or contract (default: expand)")
	all := flag.Bool("all", false, "apply every migration phase through the target version; requires --acknowledge-contract")
	targetVersion := flag.String("target-version", "", "target Mastodon release: 4.3.23, 4.4.22, 4.5.15, or 4.6.6 (default: current application release)")
