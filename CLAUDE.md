...
- `MEILI_LIBRARY_ONLY`: インデックス対象を絞り込み（true/false、デフォルト: false）

### MEILI_LIBRARY_ONLYモード

`MEILI_LIBRARY_ONLY=true` を設定すると、インデックス対象を以下に絞り込むことができます：

| MEILI_LIBRARY_ONLY | インデックス対象 |
|-------------------|-----------------|
| `false` (デフォルト) | ローカル投稿 + リモートのpublic投稿 |
| `true` | ローカル投稿 + ローカルユーザーにインタラクションされたリモート投稿 |

**インタラクションの定義：**
- お気に入り（Favourite）
- ブースト（Reblog）
- ブックマーク（Bookmark）
- メンション（Mention）← ローカルユーザーへのメンション

このモードは、大量のリモート投稿によるインデックスサイズの増大やインデックス速度の低下を防ぐ場合に有用です。

**注意事項：**
- このモードを有効化/無効化した場合、インデックスの再構築が必要です（`rake meilisearch:deploy`）
- インタラクションの有無はインデックス作成時に判定されます。既存のインデックス済み投稿に対する新しいインタラクションは自動的には反映されません
- インタラクション（お気に入り、ブックマーク等）を解除しても、該当の投稿はインデックスから自動削除されません。定期的なインデックス再構築で整理されます
- 投稿自体が削除された場合は、インデックスからも自動的に削除されます
