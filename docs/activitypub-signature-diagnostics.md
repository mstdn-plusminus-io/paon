...
診断のために本文を追加でログ出力したり、秘密鍵を記録したりしません。本文は従来どおり Asynq のペイロードに残ります。診断情報の採取のための追加の HTTP 取得は行わず、原文の再計算には内蔵・キャッシュ済みコンテキストだけを使います。この診断によって認証結果や再試行の扱いは変わりません。

## 署名不一致時の送信元再取得（任意）

参加を承認済みのリレーから転送され、署名検証に失敗した `Create`・`Update`・`Delete` を送信元への HTTPS 取得で確認し直すには、次の設定を使います。Linked Data 署名の欠落や、その署名者の取得失敗も対象です。既定では無効です。値は `1` または `true` のときだけ有効になります。

```dotenv
PAON_ALLOW_UNVERIFIED_ACTIVITY_REFETCH=1
```

Asynq の `activitypub:processing` を処理するワーカープロセスに設定し、ワーカーを再作成・再起動してください。標準の `docker-compose.yml` では `.env.production` が Web とワーカーの両方に渡されます。既存の失敗タスクも再試行時の設定に従います。

この設定は、受信本文の署名検証を成功扱いにする設定ではありません。HTTP 署名の確認、ドメインの制限、取得先のプライベートネットワークへのアクセス制限は維持し、送信元で確認できた内容だけを適用します。署名の不一致そのものは修復されず、送信元から確認できなければタスクは引き続き失敗します。

取得先は対象 object の URI で、actor と同じ HTTPS origin であることを確認します。Activity ID を取得先にしないため、`#updates/...` のようなフラグメント付き Activity ID にも対応します。

| Activity | 再取得時の確認と処理 |
| --- | --- |
| `Create` | 投稿を取得し、ID と `attributedTo` を確認して、取得した投稿から作成処理を組み立てます。 |
| `Update` | 投稿は ID と `attributedTo`、actor プロフィールは actor URI と一致する ID を確認し、取得した現在の内容を更新に使います。 |
| `Delete` | 対象 URI の直接の `410 Gone`、または対象 ID と一致する `Tombstone` を確認します。保存済みの対象は actor がその所有者であることも確認して削除します。未保存の対象は新たな Tombstone を保存せず、処理済みとして終了します。 |

`Create`・`Update` は受信時点の古い本文を再現する処理ではなく、取得できた現在の内容を取り込みます。受信した未検証本文の内容や署名は更新データとして引き継ぎません。`Delete` では `404`、`403`、`5xx`、リダイレクト先での `410`、削除されていない通常の object を削除の証拠にせず、タスクを失敗させます。

この設定で追加する対象は上記の 3 種類です。既存の署名無し `Announce` の再取得処理は、この設定とは独立しています。

再取得して送信元を確認できた場合は、ワーカーに `level=WARN event=activitypub_unverified_activity_refetched` を記録します。タスク ID、キュー、対象 URI、元の署名エラーを含み、`outcome=refetched` または `outcome=untracked_delete` で確認できます。後続処理まで成功したタスクは失敗タスクに残らないため、この場合の調査にはワーカーのログを使ってください。再取得にも失敗した場合は、Asynq の最終エラーに再取得のエラーと元の署名エラーの両方が残ります。
