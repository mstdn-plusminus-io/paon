...
OpenTelemetry collector setup, service-role names, trace propagation,
sampling, redaction, StatsD coexistence, and release security evidence are
documented in `docs/paon-go-observability.md`. Run `paon --check-config` in
both the web and worker containers before enabling OTLP export.
...
| federation self-destruct                     | `paon-admin self-destruct dry-run`, then interactive `self-destruct start`; see `docs/paon-self-destruct.md`                                                          |
...
- maintenance duplicate fixers, upgrade fixers, and emergency recovery;
